...
def jouer():
    '''
    Paramètre : aucun
    Résultat : demande au joueur le nombre de parties souhaitées, lance le jeu le nombre de fois désirées et affiche un score
    '''
    nb=int(input('Combien de parties voulez vous faire ? '))
    score=0
    for i in range(nb):
        if pendre():
            score+=1
        print('===================================================')
        print('Votre score est de ' + str(score) +'/' + str(i+1))
        print('===================================================')
    print('Votre score final est de ' + str(score) +'/' + str(nb))
